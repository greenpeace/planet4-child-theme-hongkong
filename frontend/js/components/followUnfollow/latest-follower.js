 ...
 */
export const filterDuplicatesCollections = function(
  newCollections,
  existingCollections
) {
  return newCollections.filter(
    newCollection =>
      existingCollections.find(
        existingCollection => existingCollection.slug == newCollection.slug
      ) === undefined
  );
};
  ...
  if (
    ...
    $.ajax({
      ...
      success: function(data) {
        ...
        followingResults = filterDuplicatesCollections(
          followingResults,
          existingCollections
        );
        ...
        const buildArticleRowContainer = template(
          articleRowContainer[0].innerHTML
        );
        ...
        let trendingCollectionsDynamic = followingResults.map(collection => {
          ...
          let trendingCollectionsContainer = buildArticleRowContainer(
            collection
          );
        ...
        $(trendingCollectionsDynamic).insertBefore(
          $('.section-article-row').eq(0)
        );
